...
- **Modern Dashboard** - Dark-themed UI with real-time pipeline visualization
...
- [x] Document upload & text extraction (PDF/DOCX)
- [x] Enhanced database schema (Component, Test models)
- [x] AI Agent Pipeline (Parser, Analyzer, Matcher, Scorer, Response)
- [x] LangGraph workflow orchestration
- [x] OpenRouter LLM integration (multi-model support)
- [x] Terminal logging for agent workflow visualization
- [x] Robust JSON parsing for LLM responses
- [x] Modern dashboard UI with dark theme
- [x] Real-time pipeline visualization
- [x] Results display with match scores
- [x] Proposal generation & export

### Recent Updates (v1.1)
- **Switched to OpenRouter API** - Now supports multiple LLM providers through a single API
- **Default model: x-ai/grok-4.1-fast:free** - Free model with good performance
- **Added terminal logging** - Comprehensive colored output for debugging
- **Robust JSON extraction** - Handles malformed LLM responses with multiple fallback strategies
- **Improved error handling** - Better error messages and debug output

### Future Enhancements
- [ ] Vector embeddings for semantic search
- [ ] Confidence scoring with HITL review
- [ ] Background job processing
- [ ] User authentication
- [ ] Multiple proposal templates
- [ ] Historical RFP analytics
